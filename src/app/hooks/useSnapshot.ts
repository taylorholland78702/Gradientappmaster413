...
export function useSnapshot(params: SnapshotParams) {
  ...
  const buildSnapshot = useCallback(() => {
    return {
      ...
      cubismFacetSize, cubismOffset,
      popTiles, popHueShift,
      futurismEchoes, futurismSpread,
      datamoshBlockSize, datamoshAmount,
  ...
  const applySnapshot = useCallback((snapshot: any) => {
    ...
    setDatamoshBlockSize(snapshot.datamoshBlockSize ?? 16);
    setDatamoshAmount(snapshot.datamoshAmount ?? 0.3);
    setFuturismEchoes(snapshot.futurismEchoes ?? 5);
    setFuturismSpread(snapshot.futurismSpread ?? 14);
    setPopTiles(snapshot.popTiles ?? 3);
    setPopHueShift(snapshot.popHueShift ?? 120);
    setCubismFacetSize(snapshot.cubismFacetSize ?? 28);
    setCubismOffset(snapshot.cubismOffset ?? 0.4);
