...
export function useSnapshot(params: SnapshotParams) {
  const {
    ...
    marksCount, marksSize, marksDecay,
    ...
    grainType, gridCellAngleStep, gridColumns, gridHardEdge, setGridHardEdge, gridMode, setGridMode, gridRotation, gridRows, gridShapeSize, gridSides,
    ...
    noiseAnimTime, setNoiseAnimTime, noiseSpeed, setNoiseSpeed, radialAnimTime, setRadialAnimTime, radialSpeed, setRadialSpeed,
    ...
    setContrastBeatEnabled, setDigitalNoiseIntensity, setDitherLevels, setDitherType, setDitherScale,
    ...
    setEmojiRotateSpeed, setEmojiSize, setEmojiSizeVariation, setFadeDirection, setFadeMode, setFeedbackDecay, setFeedbackRotation,
    ...
    setHatchLayers, setHatchSpacing, setHatchResponse, setHatchAnimTime, setHatchSpeed,
    ...
    hatchLayers, hatchSpacing, hatchResponse, hatchAnimTime, hatchSpeed,
  ...
  const buildSnapshot = useCallback(() => {
    ...
    waveInterferenceAnimTime, meshWireframeAnimTime,
      ...
      particlesCount, particlesSpeed, particlesSize, particlesTrail, particlesGravity, particlesSides, particlesMode, marksCount, marksSize, marksDecay,
      tilingSize, tilingSymmetry, tilingComplexity, tilingRotation, tilingRowOffset,
    waveInterferenceSourceCount, waveInterferenceFrequency, waveInterferenceSpeed,
    meshWireframeGridSize, meshWireframeJitter, meshWireframeLineWidth,
      fireworksCount, fireworksParticleCount, fireworksTrailFade,
      ...
      noiseType, noiseWarp,
      noiseAnimTime, noiseSpeed,
      radarBeamWidth, radarFadeLength, radarSweepAngle,
      radialSizeScale,
      radialHardEdge,
      radialAnimTime, radialSpeed,
      ...
      chromaticOffset, fisheyeStrength, grainIntensity, grainSize, gessoWhiteness, gessoTexture, gessoResponse, blurMotionAmount,
    ...
    meshWireframeGridSize, meshWireframeJitter, meshWireframeLineWidth,
      ...
      metaballCount, metaballSize, metaballSpeed,
      mirrorMode, mirrorTileCount,
      noiseType, noiseWarp, noiseAnimTime, noiseSpeed,
      radarBeamWidth, radarFadeLength, radarSweepAngle,
      radialSizeScale, radialHardEdge, radialAnimTime, radialSpeed,
      slitScanDirection, slitScanIntensity, slitScanHistory,
      truchetSize, truchetThickness, truchetVariation,
      vignetteSoftness,
      voronoiCellCount, voronoiDistortion,
  ...
  const applySnapshot = useCallback((snapshot: any) => {
    ...
    setGessoWhiteness(snapshot.gessoWhiteness ?? 0.85);
    setGessoTexture(snapshot.gessoTexture ?? 0.4);
    setGessoResponse(snapshot.gessoResponse ?? 0.5);
    ...
    setShapesMode(snapshot.shapesMode ?? 'polygon');
    setTurrellAnimTime(snapshot.turrellAnimTime ?? 0);
    setTurrellSpeed(snapshot.turrellSpeed ?? 1);
    setTurrellGlow(snapshot.turrellGlow ?? 0.4);
    ...
    setHatchResponse(snapshot.hatchResponse ?? 0.5);
    setHatchAnimTime(snapshot.hatchAnimTime ?? 0);
    setHatchSpeed(snapshot.hatchSpeed ?? 1);
    ...
    setGridMode(snapshot.gridMode ?? 'classic');
    ...
    setImpastoStrength(snapshot.impastoStrength ?? 3);
    setImpastoLightAngle(snapshot.impastoLightAngle ?? 135);
    setBrushStrokesSize(snapshot.brushStrokesSize ?? 14);
    setBrushStrokesLength(snapshot.brushStrokesLength ?? 2.2);
    setWatercolorBleed(snapshot.watercolorBleed ?? 3);
    setWatercolorGrain(snapshot.watercolorGrain ?? 0.4);
    ...
    setFadeMode(snapshot.fadeMode ?? 'angle');
    ...
    setParticlesMode(snapshot.particlesMode ?? 'drift');
    setMarksCount(snapshot.marksCount ?? 8);
    setMarksSize(snapshot.marksSize ?? 40);
    setMarksDecay(snapshot.marksDecay ?? 0.94);
    ...
    setNoiseAnimTime(snapshot.noiseAnimTime ?? 0);
    setNoiseSpeed(snapshot.noiseSpeed ?? 1);
    ...
    setRadialAnimTime(snapshot.radialAnimTime ?? 0);
    setRadialSpeed(snapshot.radialSpeed ?? 1);
