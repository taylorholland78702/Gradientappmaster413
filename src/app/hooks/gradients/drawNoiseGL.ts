...
export function drawNoiseGL(P: any): CanvasGradient | undefined {
  ...
  gl.uniform1f(gl.getUniformLocation(program, 'uScale'), baseNoiseScale);
  gl.uniform1f(gl.getUniformLocation(program, 'uRotCos'), noiseRotCos);
  gl.uniform1f(gl.getUniformLocation(program, 'uRotSin'), noiseRotSin);
  gl.uniform1f(gl.getUniformLocation(program, 'uDirection'), noiseDirection);
  gl.uniform1f(gl.getUniformLocation(program, 'uWarp'), noiseWarp);
  gl.uniform1i(gl.getUniformLocation(program, 'uOctaves'), Math.max(1, Math.min(MAX_OCTAVES, Math.round(noiseOctaves))));
  gl.uniform1i(gl.getUniformLocation(program, 'uNoiseType'), noiseTypeIdx);
  gl.uniform1f(gl.getUniformLocation(program, 'uColorShift'), noiseColorShift);
  gl.uniform1f(gl.getUniformLocation(program, 'uBassBoost'), noiseBassBoost);
  gl.uniform1f(gl.getUniformLocation(program, 'uMaxDist'), maxNoiseDist);
